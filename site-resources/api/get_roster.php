...
if (isset($_POST["pid"])){
  $pid = $_POST["pid"];
  $count = 0;
  if ($stmt = $mysqli->prepare("SELECT registration.*, programs.* FROM registration JOIN programs ON registration.pid=programs.pid WHERE registration.status > 0 AND registration.pid = ? ORDER BY id ASC")) {
