...
$name = "control";
$css = "control.css";
$title = "National Control Systems";
$email = "@ncs.com";
$vendor = 1;
$description = "The nation's #1 supplier of robotics and industial control systems, since 1970.";
$featured_title = "VEX Cortex Microcontroller";
$featured_description = "The VEX ARM® Cortex®-based Microcontroller coordinates the flow of   information and power on the robot. All other electronic system components must interface to the  Microcontroller.";
$featured_pid = 1;
$slogan = "<h1>A Global Leader in Test, Measurement and Control Solutions</h1>";
...
if (isset($_GET["product"])) {
  ...
  if ($product == "view_all") {
    ...
    $products = array();
    $products = get_products(-1, $mysqli, "control");
    ...
    for($i = 0; $i < count($products); $i++) {
      ...
      echo "<div class = 'product-entry'><div class = 'left-float'><h2><a href = './index.php?product=".$products[$i]["prid"]."'>".$products[$i]["name"]."</a></h2>".$products[$i]["description"]."</div><div class ='right-float'><b>P/N: ".$products[$i]["pn"]."</b><br>Price: $".$products[$i]["price"]."<br><span style = 'color:green;font-weight:bold;'>".$status."</span><br><input type = 'number' id = '".$products[$i]["prid"]."' min = '0' style = 'width: 50px;height:30px;padding:5px;'/><button class = 'addcart' data-prid = '".$products[$i]["prid"]."' data-name = '".$products[$i]["name"]."' data-cost = '".$products[$i]["price"]."' data-pn = '".$products[$i]["pn"]."'>Add to Cart</button></div></div>";
    }
    echo "</div>";
    ...
    $products = array();
    $products = get_products($product, $mysqli, "control");
    echo "<html>";
    include("../site-resources/page/head.php");
    echo "<body>";
    include("../site-resources/page/header.php");
  ...
  if (isset($account[0]["oid"]) && $account[0]["role"] == "participant"){
    ...
    echo "<table style = 'font-size:20px'><tr class = 'headerr'><th>Order ID</th><th>Date Ordered</th><th>Group</th><th>Shipping</th></tr>";
    foreach ($account as $a) {
      echo "<tr><td>".$a["oid"]."</td><td>".$a["timestamp"]."</td><td>".$a["gid"]."</td><td>".$a['shipping']."</td></tr>";
    }
    echo "</table>";
  } else {
